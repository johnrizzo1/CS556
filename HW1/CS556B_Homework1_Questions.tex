...
\begin{enumerate}
    ...
    $|x| = \sqrt{1^2+-3^2+8^2+-5^2+-1^2} = \sqrt{100} = 10$
    ...
    This matrix $\mathbb{R}^1$ is the line.

    $\begin{bmatrix}
        20 \\ 4
    \end{bmatrix}$ does not belong to the Span $\begin{bmatrix}
        2 \\ 4
    \end{bmatrix}$ because it is not along the line $\begin{bmatrix}
        2 \\ 4
    \end{bmatrix}$ therefor it is not reachable.
\end{enumerate}
...
\begin{enumerate}
    ...
    $|u| |v| cos \theta = 10 \cdot 6 \cos{60} = 30$
...
\begin{enumerate}
  \setcounter{enumi}{8}
    \item (10 points) Consider the three columns in matrix $A$ (problem 8) to be our new basis of interest in $R^3$. If a vector $\mathbf{x} = \begin{bmatrix} 1 \\ 2 \\ 3 \end{bmatrix}$ defined on the natural basis in $R^3$ (i.e.,  $\mathbf{e_1} = \begin{bmatrix} 1 \\ 0 \\ 0 \end{bmatrix}$, $\mathbf{e_2} = \begin{bmatrix} 0 \\ 1 \\ 0 \end{bmatrix}$, $\mathbf{e_3} = \begin{bmatrix} 0 \\ 0 \\ 1 \end{bmatrix}$), how would vector \textbf{x} be represented in the basis defined by the matrix $A$ in problem 8 ?\vspace{2ex}
...
\begin{enumerate}
    ...
    $adj(A^T A)^{-1} = \begin{bmatrix} -1 & 2 \\ 1 & -1\end{bmatrix}$

    $(A^T A)^{-1} = \frac{1}{1}\begin{bmatrix}
        -1 & 2 \\
        1 & -1 
    \end{bmatrix} = \begin{bmatrix}
        -1 & 2 \\
